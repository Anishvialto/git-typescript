class AnagramFinder {
    ...
    constructor(s: string, p: string)  // initialize two variables(s,p)
    {
    ...
    findAnagrams(): number[] {
        let result: number[] = [];    // result will store final answer(starting indices).
        let slen: number = this.s.length;  // slen stores length of string s
        let plen: number = this.p.length;  // plen stores length of string p
        ...
        let pcount: number[] = new Array(26).fill(0);  // create two arrays of size 26 (for 26 english lowercase letters)
    ...
    arraysMatch(arr1: number[], arr2: number[]): boolean  // compare two arrays arr1 and arr2 element by element 
    {
